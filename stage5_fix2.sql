...
begin
    ...
    if p_decision = 'accepted' then
        ...
        -- Keep application in offer_received status until Placement HR confirms placement
        -- But log the student acceptance in application history
        insert into public.application_status_history (application_id, old_status, new_status, changed_by, metadata)
        values (v_offer.application_id, 'offer_received', 'offer_received', v_actor, jsonb_build_object('offer_id', v_offer.id, 'student_decision', 'accepted'));
...
begin
    ...
    insert into public.application_status_history (application_id, old_status, new_status, changed_by, metadata)
    values (v_offer.application_id, 'offer_received', 'placed', v_actor, jsonb_build_object('offer_id', v_offer.id, 'placement_id', v_placement_id));
