...
async function assertApprovalPrerequisites(userId: string): Promise<void> {
  ...
  if (names.includes("student")) {
    ...
    if (!student || student.verification_status !== "verified") {
      throw new Error(
        "Student must be verified against HRAnalyst before approval",
      );
    }
  }
  if (names.includes("client_hr")) {
    ...
    if (
      ...
      throw new Error(
        "Client HR company must be verified and active before approval",
      );
    }
  }
}
export async function changeUserStatus(
  ...
  if (
    admin.id === userId &&
    ["rejected", "suspended", "inactive"].includes(nextStatus)
  ) {
