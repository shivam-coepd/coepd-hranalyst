...
export async function buildPlacementsCsv() {
  ...
  const rows = (data ?? []).map((placement) => {
    ...
    const job = Array.isArray(placement.jobs)
      ? placement.jobs[0]
      : placement.jobs;

    return {
      PlacementCode: placement.placement_code,
      ...
      Designation: placement.placed_designation,

      Location: placement.joining_location ?? "",

      AnnualCTC: placement.annual_ctc ?? "",
  ...
  return buildCsv(
    (data ?? []).map((application) => {
      ...
      return {
        ...
        SelectedAt: application.selected_at ?? "",

        OfferReceivedAt: application.offer_received_at ?? "",

        PlacedAt: application.placed_at ?? "",
      };
    }),
  );
}

export async function buildFeedbackSlaCsv() {
  const { data, error } = await supabaseAdmin
  ...
  return buildCsv(
    (data ?? []).map((row) => ({
      ...
      FeedbackSubmittedAt: row.submitted_at ?? "",

      FeedbackDueAt: row.feedback_due_at,

      SLAStatus: row.sla_status,

      ElapsedHours: Number(row.elapsed_hours ?? 0).toFixed(2),
    })),
  );
}
