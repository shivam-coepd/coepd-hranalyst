...
import { uploadOfferFile, deleteOfferFile } from "./offer-file.service";
export async function registerOffer({
  ...
  const parsed = registerOfferSchema.parse(input);
  const uploaded = await uploadOfferFile({
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("register_offer", {
      ...
      p_department: parsed.department ?? null,
      p_employment_type: parsed.employmentType ?? null,
      p_joining_location: parsed.joiningLocation ?? null,
      p_annual_ctc: parsed.annualCtc ?? null,
      p_currency: parsed.currency,
      p_joining_date: parsed.joiningDate ?? null,
      p_offer_date: parsed.offerDate ?? null,
      p_offer_valid_until: parsed.offerValidUntil ?? null,
      p_probation_period_months: parsed.probationPeriodMonths ?? null,
      p_notice_buyout_available: parsed.noticeBuyoutAvailable ?? null,
      p_notes: parsed.notes ?? null,
      p_bucket_name: uploaded.bucketName,
      p_storage_path: uploaded.storagePath,
      p_original_file_name: uploaded.originalFileName,
...
export async function decideOffer(input: StudentOfferDecisionInput) {
