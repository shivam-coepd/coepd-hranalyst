...
export interface CreateUserState { success: boolean; message: string; fieldErrors?: Record<string, string[]> }
...
export async function createUserAction(
  ...
  const parsed = createUserSchema.safeParse({
    ...
    phone: formData.get("phone"), role: formData.get("role"), enrollmentId: formData.get("enrollmentId"), companyId: formData.get("companyId"),
  });

  if (!parsed.success) {
    return { success: false, message: "Please correct the highlighted information", fieldErrors: parsed.error.flatten().fieldErrors };
  }
