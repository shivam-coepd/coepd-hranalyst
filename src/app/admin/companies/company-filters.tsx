...
export function CompanyFilters({
  ...
  const applyFilters = useCallback((overrideStatus?: string, overrideIsActive?: string) => {
    ...
    if (finalIsActive) {
      params.set("isActive", finalIsActive);
    } else {
  ...
  }, [search, status, isActive, pathname, router, searchParams]);
  ...
  const handleStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setStatus(newStatus);
    applyFilters(newStatus, isActive);
  };
      ...
      <Sheet>
        ...
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>
              Narrow down the companies list by applying additional filters.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="p-6">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-foreground tracking-tight">Active Status</label>
                {isActive && (
                  <button 
                    onClick={() => {
                      setIsActive("");
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { id: "", label: "All Status", icon: "🏢" },
                  { id: "true", label: "Active", icon: "🟢" },
                  { id: "false", label: "Inactive", icon: "🔴" },
                ].map((s) => (
                  <button
                    key={s.id}
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className={`text-sm ${isActive === s.id ? "font-semibold text-primary" : "font-medium text-foreground"}`}>
                      {s.label}
                    </span>
                  </button>
                ))}
