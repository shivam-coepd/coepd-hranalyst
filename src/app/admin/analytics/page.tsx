...
export default async function AdminAnalyticsPage() {
  ...
  const chartData = [
    {
      name: "Applications",
      value: Number(funnel.total_applications ?? 0),
    },

    {
      name: "Verified",
      value: Number(funnel.verified_or_beyond ?? 0),
    },

    {
      name: "Submitted",
      value: Number(funnel.submitted_to_client ?? 0),
    },

    {
      name: "Shortlisted",
      value: Number(funnel.shortlisted ?? 0),
    },

    {
      name: "Interview",
      value: Number(funnel.interview_stage ?? 0),
    },

    {
      name: "Selected",
      value: Number(funnel.selected ?? 0),
    },

    {
      name: "Placed",
      value: Number(funnel.placed ?? 0),
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard label="Placements" value={placement.total_placements ?? 0} />
        ...
        <KpiCard label="Open Operational Alerts" value={analytics.openAlerts} />

        <KpiCard label="Feedback Pending" value={analytics.feedback.pending} />
        ...
        <KpiCard
          label="Average CTC"
          value={`₹${Number(placement.average_ctc_inr ?? 0).toLocaleString(
            "en-IN",
          )}`}
        />
      </div>
