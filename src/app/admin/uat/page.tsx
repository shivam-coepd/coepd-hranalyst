...
export default async function UatRunsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold">UAT Runs</h1>
          ...
          <tbody>
            {runs.map((run) => (
              <tr key={run.id} className="border-t">
                <td className="px-4 py-3 font-medium">{run.run_code}</td>

                <td className="px-4 py-3">{run.release_version}</td>

                <td className="px-4 py-3">
                  {run.passed_tests}/{run.total_tests}
                </td>

                <td className="px-4 py-3">{run.failed_tests}</td>
