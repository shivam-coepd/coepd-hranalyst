...
async function check(
  ...
  return {
    ...
    passed: expectedStatus.includes(response.status),
...
async function main() {
  const checks = await Promise.all([
    ...
    check("Readiness", "/api/readiness", [200]),

    check("Login", "/login", [200]),

    check("Admin Protection", "/admin", [302, 307, 308]),

    check("Student Protection", "/student", [302, 307, 308]),

    check("Client Protection", "/client", [302, 307, 308]),

    check("Placement HR Protection", "/placement-hr", [302, 307, 308]),
  ]);
